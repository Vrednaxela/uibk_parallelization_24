...
grid_3D mpi_handler::make_local_grid(const grid_3D &global_grid) {
	...
	// TBD by students
	bound_up_global[0] = global_grid.z_grid.get_left(global_grid.get_num_cells(0));
	bound_up_global[1] = global_grid.z_grid.get_left(global_grid.get_num_cells(1));
	bound_up_global[2] = global_grid.z_grid.get_left(global_grid.get_num_cells(2));
	...
	for(int i_dim=0; i_dim<3; ++i_dim) {
		...
		// TBD by students
		bound_up_local[i_dim] = bound_low_global[i_dim] + spatial_shift[i_dim] + num_cells_local[i_dim]*size_cell[i_dim];
	}
